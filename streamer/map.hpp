...
public:
    ...
    auto stream(streamer_t<T> &st, std::vector<T> &values) {
        return as_map_t(std::move(k), identity<T>(), dup_throw).stream(st, values);
    }
...
public:
    ...
    template<typename T>
    auto stream(streamer_t<T> &st, std::vector<T> &values) {
        return as_multimap_t(std::move(k), identity<T>()).stream(st, values);
    }
...
auto as_multimap(KeyFunc k, ValueFunc v) {
    return as_multimap_t(std::move(k), std::move(v));
}


template<typename KeyFunc>
auto as_multimap(KeyFunc k) {
    return as_multimap_identity_t(std::move(k));
}
...
public:
    ...
    template<typename T>
    auto stream(streamer_t<T> &st, std::vector<T> &values) {
        return as_grouping_t(std::move(k), identity<T>()).stream(st, values);
    }
...
auto as_grouping(KeyFunc k, ValueFunc v) {
    return as_grouping_t(std::move(k), std::move(v));
}


template<typename KeyFunc>
auto as_grouping(KeyFunc k) {
    return as_grouping_identity_t(std::move(k));
}
