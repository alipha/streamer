...
template<typename KeyFunc, typename ValueFunc, typename Comp>
class as_multimap_t : public step_wrapper<as_multimap_t<KeyFunc, ValueFunc, Comp> > {
public:
    as_multimap_t(KeyFunc &&keyFunc, ValueFunc &&valueFunc, Comp &&comp)
        : k(std::move(keyFunc)), v(std::move(valueFunc)), c(std::move(comp)) {}

    template<typename T>
    auto stream(streamer_t<T> &, std::unique_ptr<step<T> > &s, bool &unbounded) {
        if(unbounded)
            throw unbounded_stream("cannot use as_multimap on an unbounded stream");

        using K = typename remove_ref_cv<decltype(k(*s->get()))>::type;
        using V = typename remove_ref_cv<decltype(v(*std::move(s->get())))>::type;

        std::multimap<K, V, Comp> out(c);

        while(std::optional<T> value = s->get()) {
            K key = k(*value);
            out.insert(std::move(key), v(*std::move(value)));
        }
...
template<typename KeyFunc, typename ValueFunc>
class as_multimap_less_t : public step_wrapper<as_multimap_less_t<KeyFunc, ValueFunc> > {
public:
    as_multimap_less_t(KeyFunc &&keyFunc, ValueFunc &&valueFunc)
        : k(std::move(keyFunc)), v(std::move(valueFunc)) {}

    template<typename T>
    auto stream(streamer_t<T> &st, std::unique_ptr<step<T> > &s, bool &unbounded) {
        using K = typename remove_ref_cv<decltype(k(*s->get()))>::type;
        return as_multimap_t(std::move(k), std::move(v), std::less<K>()).stream(st, s, unbounded);
    }
...
template<typename KeyFunc>
class as_multimap_identity_t : public step_wrapper<as_multimap_identity_t<KeyFunc> > {
public:
    ...
    auto stream(streamer_t<T> &st, std::unique_ptr<step<T> > &s, bool &unbounded) {
        using K = typename remove_ref_cv<decltype(k(*s->get()))>::type;
        return as_multimap_t(std::move(k), identity<T>(), std::less<K>()).stream(st, s, unbounded);
    }
...
template<typename KeyFunc, typename ValueFunc, typename Comp>
auto as_multimap(KeyFunc k, ValueFunc v, Comp c) {
    return detail::as_multimap_t(
        detail::member_mapper(std::move(k)), 
        detail::member_mapper(std::move(v)), 
        std::move(c) 
    );
...
auto as_multimap(KeyFunc k, ValueFunc v) {
    return detail::as_multimap_less_t(
        detail::member_mapper(std::move(k)), 
        detail::member_mapper(std::move(v))
   );
}
...
auto as_multimap(KeyFunc k) {
    return detail::as_multimap_identity_t(detail::member_mapper(std::move(k)));
}
